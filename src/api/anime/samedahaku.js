...
// ──────────────────────────────────────────────────────────────
// 4️⃣ GET STREAMING LINK (UNTUK EPISODE SPECIFIC)
// ──────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────
// 5️⃣ GET LATEST EPISODE STREAM (OTOMATIS AMBIL EPISODE TERBARU)
// ──────────────────────────────────────────────────────────────
async function getLatestEpisodeStream(animeUrl) {
    try {
        // Ambil halaman anime
        ...
        // Cari episode list
        $('.episode-list li, .daftar-episode li, .eps-item, .listeps li').each((i, el) => {
            const epLink = $(el).find('a').attr('href');
            const epTitle = $(el).find('a').text().trim();
            const epDate = $(el).find('.date, .eps-date').text().trim();
            
            if (epLink && epTitle && !epTitle.toLowerCase().includes('komentar')) {
                episodes.push({
        ...
        if (episodes.length === 0) {
            $('.lstepx a, .episodelist a, .eps a').each((i, el) => {
        ...
        if (episodes.length === 0) {
            throw new Error('Tidak ditemukan episode untuk anime ini');
        }
        
        const latestEpisode = episodes[0];
        
        // Ambil player URL dari episode tersebut
        const playerResult = await getStreamingUrl(latestEpisode.url);
        
        return {
            anime_url: animeUrl,
            episode: latestEpisode,
            player_url: playerResult.player_url,
            download_links: playerResult.download_links
        };
        
    } catch (error) {
        throw new Error(`Gagal ambil episode terbaru: ${error.message}`);
    }
}
...
module.exports = (app) => {
    ...
    // ─── STREAM EPISODE (LANGSUNG DARI URL EPISODE) ─────────────
    ...
    // ─── STREAM LATEST EPISODE (OTOMATIS DARI URL ANIME) ─────────
    app.get('/anime/samehadaku/stream/latest', async (req, res) => {
        const { url } = req.query;
        ...
        if (!url) {
            return res.status(400).json({
                status: false,
                creator: 'AxlyDev',
                error: 'Parameter "url" diperlukan (URL anime dari Samehadaku)'
            });
        }
        
        if (!url.includes('samehadaku.how')) {
            return res.status(400).json({
                status: false,
                creator: 'AxlyDev',
                error: 'URL harus dari samehadaku.how'
            });
        }
        
        try {
            const result = await getLatestEpisodeStream(url);
