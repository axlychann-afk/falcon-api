...
module.exports = function (app) {
  ...
  app.get('/anime/kompi/home', async (req, res) => {
    ...
    try {
      ...
      res.json({ status: true, creator: 'AxlyChann', result: scrapedData });
    } catch (err) {
      res.status(500).json({ status: false, creator: 'AxlyChann', message: err.message });
    }
  });
  ...
  app.get('/anime/kompi/search', async (req, res) => {
    const q = req.query.q;
    if (!q) return res.status(400).json({ status: false, creator: 'AxlyChann', message: 'Parameter q wajib diisi' });
    ...
    try {
      ...
      res.json({ status: true, creator: 'AxlyChann', result: searchResults });
    } catch (err) {
      res.status(500).json({ status: false, creator: 'AlyChann', message: err.message });
    }
  });
...
app.get('/anime/kompi/detail', async (req, res) => {
    ...
    if (!url) return res.status(400).json({ status: false, creator: 'AxlyChann', message: 'Parameter url wajib diisi' });
    ...
    try {
      ...
      res.json({ status: true, creator: 'AxlyChann', result: animeData });
    } catch (err) {
