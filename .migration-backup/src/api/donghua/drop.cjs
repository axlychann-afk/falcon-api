...
module.exports = (app) => {
  ...
  // donghub tidak punya kategori drop — dipetakan ke hiatus. 1 halaman per hit (?page=N).
  app.get('/donghua/drop', async (req, res) => {
    try {
      ...
      const url = page <= 1
        ? `${BASE_URL}/anime/?status=hiatus&order=update`
        : `${BASE_URL}/anime/page/${page}/?status=hiatus&order=update`;
      console.log(`[Drop] Scraping page ${page}...`);
      ...
      res.json({
        ...
        results: results,
        note: 'Sumber tidak punya kategori drop — dipetakan ke hiatus.'
      });
