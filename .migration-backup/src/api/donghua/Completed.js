...
module.exports = (app) => {
  ...
  // donghub tidak punya /completed/ — pakai arsip filter. 1 halaman per hit (?page=N).
  app.get('/donghua/completed', async (req, res) => {
    try {
      ...
      const url = page <= 1
        ? `${BASE_URL}/anime/?status=completed&order=update`
        : `${BASE_URL}/anime/page/${page}/?status=completed&order=update`;
      console.log(`[Completed] Scraping page ${page}...`);
